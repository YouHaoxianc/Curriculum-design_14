...
class ICACountInterface(tk.Tk):
    def __init__(self, path):
        super().__init__()
        self.title("类别无关的植物计数")
        self.geometry("960x625")
        self.resizable(False, False)
        self.Main_cv = tk.Canvas(self, width = 960, height= 625)
        self.Display_width = 420
        self.Display_height = 235
        # Init Variable
        self.COUNT_RESULT = 0
    ...
    def init_exemplar_area(self):
        RECTANGLE_BASE_X = 485
        RECTANGLE_BASE_Y = 330
        BASE_X = 500
        BASE_Y = 330

        self.Main_cv.create_rectangle(RECTANGLE_BASE_X, RECTANGLE_BASE_Y, RECTANGLE_BASE_X + 460,
                                      RECTANGLE_BASE_Y + 120)
        self.Main_cv.pack()
        Input_Image_Text = tk.Label(self, text="绘制边界框", fg='blue')
        Input_Image_Text.pack()
        Input_Image_Text['font'] = font.Font(size=15)
        Input_Image_Text.place(x=BASE_X + 16, y=BASE_Y - 8)

        self.ExempalrB = tk.Button(self, text="边界框", command=self.activate_drawing)
        self.ExempalrB['font'] = font.Font(size=15)
        self.ExempalrB.place(x=BASE_X + 33, y=BASE_Y + 46)
        self.ExempalrB["width"] = 10
        self.ExempalrB["height"] = 1
        self.ExempalrB["relief"] = tk.GROOVE

        self.ExempalrUnDoB = tk.Button(self, text="撤销", command=self.exemplar_undo)
        self.ExempalrUnDoB['font'] = font.Font(size=15)
        self.ExempalrUnDoB.place(x=BASE_X + 164, y=BASE_Y + 46)
        self.ExempalrUnDoB["width"] = 10
        self.ExempalrUnDoB["height"] = 1
        self.ExempalrUnDoB["relief"] = tk.GROOVE

        self.ExempalrResetB = tk.Button(self, text="重置", command=self.exemplar_reset)
        self.ExempalrResetB['font'] = font.Font(size=15)
        self.ExempalrResetB.place(x=BASE_X + 296, y=BASE_Y + 46)
        self.ExempalrResetB["width"] = 10
    ...
    def init_interactive_counting_area(self):
        RECTANGLE_BASE_X = 485
        RECTANGLE_BASE_Y = 485
        BASE_X = 500
        BASE_Y = 485

        # Init Layout
        self.Main_cv.create_rectangle(RECTANGLE_BASE_X, RECTANGLE_BASE_Y, RECTANGLE_BASE_X + 460, RECTANGLE_BASE_Y + 120)
        self.Main_cv.pack()
        Input_Image_Text = tk.Label(self, text="交互式计数", fg='blue')
        Input_Image_Text.pack()
        Input_Image_Text['font'] = font.Font(size=15)
        Input_Image_Text.place(x= BASE_X + 16, y = BASE_Y - 8)

        # Count Label
        count_label = tk.Label(self, text="计数结果：", fg='red')
        count_label.pack()
        count_label['font'] = font.Font(size=15)
        count_label.place(x=BASE_X + 33, y=BASE_Y + 17)
        ...
        self.count_res_label['font'] = font.Font(size=15)
        self.count_res_label.pack()
        self.count_res_label.place(x=BASE_X + 130, y=BASE_Y + 17)

        self.count_button = tk.Button(self, text="初始计数", command=self.initial_count)
        self.count_button['font'] = font.Font(size=15)
        self.count_button.place(x=BASE_X + 33, y=BASE_Y + 46)
        self.count_button["width"] = 10
        self.count_button["height"] = 1
        self.count_button["relief"] = tk.GROOVE

        self.UnDoB = tk.Button(self, text="撤销", command=self.interactive_undo)
        self.UnDoB['font'] = font.Font(size=15)
        self.UnDoB.place(x=BASE_X + 164, y=BASE_Y + 46)
        self.UnDoB["width"] = 10
        self.UnDoB["height"] = 1
        self.UnDoB["relief"] = tk.GROOVE

        self.ResetB = tk.Button(self, text="重置", command=self.interactive_reset)
        self.ResetB['font'] = font.Font(size=15)
        self.ResetB.place(x=BASE_X + 296, y=BASE_Y + 46)
        self.ResetB["width"] = 10
        self.ResetB["height"] = 1
        self.ResetB["relief"] = tk.GROOVE

        gs_str_var = tk.StringVar()
        gs_str_var.set('10')
        gs_label = tk.Label(self, text="梯度步数：")
        gs_label.pack()
        gs_label['font'] = font.Font(size=12)
        gs_label.place(x=BASE_X + 66, y=BASE_Y + 86)

        self.GS_entry = tk.Entry(self, bd=3, textvariable=gs_str_var)
        self.GS_entry['font'] = font.Font(size=12)
        self.GS_entry.place(x=BASE_X + 151, y=BASE_Y + 86)
        self.GS_entry["width"] = 4

        lr_str_var = tk.StringVar()
        lr_str_var.set('0.02')
        lr_label = tk.Label(self, text="学习率：")
        lr_label.pack()
        lr_label['font'] = font.Font(size=12)
        lr_label.place(x=BASE_X + 217, y=BASE_Y + 86)

        self.lr_entry = tk.Entry(self, bd=3, textvariable=lr_str_var)
        self.lr_entry['font'] = font.Font(size=12)
        self.lr_entry.place(x=BASE_X + 285, y=BASE_Y + 86)
        self.lr_entry["width"] = 4


    def init_image_density_result_area(self):
        # Init Image Area
        IMAGE_RECTANGLE_BASE_X = 13
        IMAGE_RECTANGLE_BASE_Y = 20
        self.Main_cv.create_rectangle(IMAGE_RECTANGLE_BASE_X, IMAGE_RECTANGLE_BASE_Y, IMAGE_RECTANGLE_BASE_X + 460, IMAGE_RECTANGLE_BASE_Y + 275)
        self.Main_cv.pack()
        Input_Image_Text = tk.Label(self, text="输入图片", fg='blue')
        Input_Image_Text.pack()
        Input_Image_Text['font'] = font.Font(size=15)
        Input_Image_Text.place(x=IMAGE_RECTANGLE_BASE_X + 16, y=IMAGE_RECTANGLE_BASE_Y - 8)
        self.Input_Image_Label = tk.Label(self, text="")
        self.Input_Image_Label.pack()
        self.Input_Image_Label.place(x=IMAGE_RECTANGLE_BASE_X + 16, y=IMAGE_RECTANGLE_BASE_Y + 18)
        self.EXEMPLAR_LIST = []
        ...
        # Init Density Area
        DENSITY_RECTANGLE_BASE_X = 485
        DENSITY_RECTANGLE_BASE_Y = 20
        self.Main_cv.create_rectangle(DENSITY_RECTANGLE_BASE_X, DENSITY_RECTANGLE_BASE_Y, DENSITY_RECTANGLE_BASE_X + 460, DENSITY_RECTANGLE_BASE_Y + 275)
        self.Main_cv.pack()
        self.Density_Label = tk.Label(self, text="")
        self.Density_Label.pack()
        self.Density_Label.place(x=DENSITY_RECTANGLE_BASE_X + 16, y=DENSITY_RECTANGLE_BASE_Y + 18)
        default_img = Image.open('default_img.jpg')
        default_img = default_img.resize((self.Display_width, self.Display_height))
        default_show = ImageTk.PhotoImage(default_img)
        self.Density_Label.configure(image=default_show)
        self.Density_Label.image = default_show
        Density_Text = tk.Label(self, text="估计密度图", fg='blue')
        Density_Text.pack()
        Density_Text['font'] = font.Font(size=15)
        Density_Text.place(x=DENSITY_RECTANGLE_BASE_X + 16, y=DENSITY_RECTANGLE_BASE_Y - 9)

        # Init Result Area
        RESULT_RECTANGLE_BASE_X = 13
        RESULT_RECTANGLE_BASE_Y = 330
        self.Main_cv.create_rectangle(RESULT_RECTANGLE_BASE_X, RESULT_RECTANGLE_BASE_Y, RESULT_RECTANGLE_BASE_X + 460, RESULT_RECTANGLE_BASE_Y + 275)
        self.Main_cv.pack()
        Input_Image_Text = tk.Label(self, text="结果可视化", fg='blue')
        Input_Image_Text.pack()
        Input_Image_Text['font'] = font.Font(size=15)
        Input_Image_Text.place(x=RESULT_RECTANGLE_BASE_X + 16, y=RESULT_RECTANGLE_BASE_Y - 8)
        self.Visual_Label = tk.Label(self, text="")
        self.Visual_Label.pack()
        self.Visual_Label.place(x=RESULT_RECTANGLE_BASE_X + 16, y=RESULT_RECTANGLE_BASE_Y + 18)
        default_img = Image.open('default_img.jpg')
        default_img = default_img.resize((self.Display_width, self.Display_height))
        default_show = ImageTk.PhotoImage(default_img)
    ...
    def init_menu_bar(self):
        ...
        self.MB.add_command(label="打开图片", command=self.Menu_Bar_Load_Image)
    ...
    def init_visual_counter(self):
        ...
        self.visual_counter.load_state_dict(torch.load('FamNet/FamNet.pth', map_location=self.device))
    ...
    def initial_count(self):
        if self.Image_path is None:
            messagebox.showinfo("", "请选择一张图片！")
            return

        if len(self.EXEMPLAR_LIST) < 3:
            messagebox.showinfo("", "请选择至少3个示例！")
            return

        if len(self.EXEMPLAR_LIST) > 3:
            messagebox.showinfo("", "对于FamNet只使用前3个示例！")
    ...
    def init_popup_menu(self):
        ...
        self.popup_menu.add_command(label='(-∞,0]', command=self.popup_menu_0)
        ...
        self.popup_menu.add_command(label='(4,∞)', command=self.popup_menu_5)
    ...
    def interactive_adaptation(self):
        ...
        inter_mask = torch.from_numpy(inter_mask).cpu()
        ...
        for step in range(0, scale_INGS):
            ...
            all_inter_mask = torch.from_numpy(all_inter_mask).cpu()
